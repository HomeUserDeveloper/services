    ...
    if request.method == "POST":
        ...
        if action == "export":
            ...
            _create_service_exchange_log(
                ...
                dry_run=False,
                summary=summary,
                message=f"Формат: {export_format}",
            )
...
def _paginate_report_queryset(request: HttpRequest, queryset, default_per_page: str = "50"):
...
def _sync_product_model_legacy_field_from_characteristic(model: ProductModel, characteristic_code: str) -> None:
    ...
    value = ""
    record = (
        ProductModelCharacteristic.objects.select_related("characteristic_type")
        .filter(product_model=model, characteristic_type__code=characteristic_code)
        .first()
    )
    if record:
    ...
    if getattr(model, field_name, "") != value:
        ...
        model.save(update_fields=[field_name, "updated_at"])
...
def product_model(request):
    ...
    models_qs = ProductModel.objects.select_related("brand", "category").all()
    if normalized_query:
...
def consumable(request):
    ...
    consumables_qs = Consumable.objects.select_related("brand", "category").all()
    if normalized_query:
    ...
    if category_filter:
        try:
            ...
            pass
    if device_type_filter:
        consumables_qs = consumables_qs.filter(device_type=device_type_filter)
    ...
    all_categories = ProductCategory.objects.all().order_by("group", "name")
    all_device_types = (
        Consumable.objects.exclude(device_type="")
        .values_list("device_type", flat=True)
        .distinct()
        .order_by("device_type")
    )
...
def part(request):
    ...
    parts_qs = Part.objects.select_related("brand", "category").all()
    if normalized_query:
    ...
    if category_filter:
        try:
            ...
            pass
    if device_type_filter:
        parts_qs = parts_qs.filter(device_type=device_type_filter)
    ...
    all_categories = ProductCategory.objects.all().order_by("group", "name")
    all_device_types = (
        Part.objects.exclude(device_type="")
        .values_list("device_type", flat=True)
        .distinct()
        .order_by("device_type")
    )
...
def report_part(request):
    ...
    items_qs = Part.objects.select_related("brand", "category").all()
    if query_lc:
    ...
    if device_type_filter:
        items_qs = items_qs.filter(device_type=device_type_filter)
    ...
    if export_format == "excel_list":
        ...
        headers = ["Наименование", "Артикул", "Бренд", "Категория", "Тип устройства", "Сайт"]
        for col, header in enumerate(headers, start=1):
        ...
        for item in items_qs:
            ...
            sheet.cell(row=row, column=5, value=item.device_type or "-")
    ...
    if export_format == "excel":
        ...
        sheet["B5"] = selected_item.device_type or "-"

        row = 7
        sheet.cell(row=row, column=1, value="Привязки к технике").font = Font(bold=True)
        row += 1
        headers = ["Техника", "Бренд", "Категория", "Артикул"]
        for col, header in enumerate(headers, start=1):
            sheet.cell(row=row, column=col, value=header).font = Font(bold=True)
        row += 1
    ...
    context = {
        ...
        "all_brands": Brand.objects.all().order_by("name"),
        "all_categories": ProductCategory.objects.all().order_by("group", "name"),
        "all_device_types": Part.objects.exclude(device_type="").values_list("device_type", flat=True).distinct().order_by("device_type"),
        "brand_filter": brand_filter,
        "category_filter": category_filter,
        "device_type_filter": device_type_filter,
        "sort": sort,
...
def report_consumable(request):
    ...
    items_qs = Consumable.objects.select_related("brand", "category").all()
    if query_lc:
        items_qs = items_qs.annotate(
            name_lc=Lower("name"),
            site_lc=Lower("site"),
    ...
    context = {
        ...
        "all_brands": Brand.objects.all().order_by("name"),
        "all_categories": ProductCategory.objects.all().order_by("group", "name"),
        "all_device_types": Consumable.objects.exclude(device_type="").values_list("device_type", flat=True).distinct().order_by("device_type"),
        "brand_filter": brand_filter,
        "category_filter": category_filter,
        "device_type_filter": device_type_filter,
        "sort": sort,
...
def report_work_directory(request):
    ...
    if export_format == "excel_list":
        ...
        headers = ["Код", "Наименование", "Цена", "Расходники", "Запчасти"]
        for col, header in enumerate(headers, start=1):
            sheet.cell(row=3, column=col, value=header).font = Font(bold=True)
    ...
    if export_format == "excel":
        ...
        sheet.cell(row=row, column=1, value="Расходные материалы").font = Font(bold=True)
        row += 1
        for col, header in enumerate(["Наименование", "Артикул", "Бренд", "Количество"], start=1):
            sheet.cell(row=row, column=col, value=header).font = Font(bold=True)
        row += 1
...
def report_characteristics(request):
    ...
    if export_format == "excel_list":
        ...
        headers = ["Техника", "Бренд", "Характеристика", "Значение"]
        for col, header in enumerate(headers, start=1):
            sheet.cell(row=3, column=col, value=header).font = Font(bold=True)
...
def repair_document_edit(request, document_id=None):
    ...
    if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if status_changed:
                    with transaction.atomic():
                        ...
                        # История: у нового статуса остаются текущие списки,
                        # а у предыдущего восстанавливается снимок при открытии формы.
                    ...
                    # Пользователь сохранил документ без смены статуса:
                    # обновляем базовый снимок для дальнейшей истории.
        ...
        # Снимок фиксируется один раз за сессию редактирования документа.
...
def client_equipment(request):
    ...
    if request.method == "POST":
        ...
        # Передаём organization_id в форму для валидации
