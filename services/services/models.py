...
def catalog_attachment_upload_to(instance, filename):
    ...
    return f"catalog_attachments/{instance._meta.model_name}/{uuid.uuid4().hex}{extension}"


class CatalogReferenceMixin(models.Model):
...
class ProductModel(CatalogReferenceMixin, models.Model):
...
class Consumable(CatalogReferenceMixin, models.Model):
    name = models.CharField("Наименование", max_length=255)
    site = models.URLField("Сайт", blank=True)
    category = models.ForeignKey(
        ProductCategory,
...
class ConsumableCharacteristic(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["consumable", "characteristic_type"], name="uniq_consumable_characteristic_type"),
        ]

    def __str__(self):
        return f"{self.consumable}: {self.characteristic_type.name} = {self.value}"
...
class Part(CatalogReferenceMixin, models.Model):
    name = models.CharField("Наименование", max_length=255)
    site = models.URLField("Сайт", blank=True)
    category = models.ForeignKey(
        ProductCategory,
...
class PartCharacteristic(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["part", "characteristic_type"], name="uniq_part_characteristic_type"),
        ]

    def __str__(self):
        return f"{self.part}: {self.characteristic_type.name} = {self.value}"


class ProductModelAttachment(CatalogAttachmentBase):
    product_model = models.ForeignKey(
        ProductModel,
        on_delete=models.CASCADE,
...
class ConsumableAttachment(CatalogAttachmentBase):
    consumable = models.ForeignKey(
        Consumable,
        on_delete=models.CASCADE,
        related_name="attachments",
...
class PartAttachment(CatalogAttachmentBase):
    part = models.ForeignKey(
        Part,
        on_delete=models.CASCADE,
        related_name="attachments",
